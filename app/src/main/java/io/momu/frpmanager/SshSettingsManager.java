...
public class SshSettingsManager {
    ...
    public void saveSshSettings(String host, int port, String username, String password) {
        ...
        editor.putString(KEY_PASSWORD, password); // Note: In a production environment, passwords should be encrypted.
    ...
    /**
     * Sets whether the app is allowed to manage the server's firewall.
     * @param manage True to allow management, false otherwise.
     */
    ...
    /**
     * Checks if the user has authorized the app to manage the firewall.
     * @return True if authorized, false otherwise (default is false for security).
     */
    ...
    /**
     * Checks if SSH configuration exists and is complete.
     * @return True if all necessary information is configured.
     */
    ...
    public String getFrpServerAddr() {
        // Defaults to the SSH host if no specific FRP server address is set.
        return sharedPreferences.getString(KEY_FRP_SERVER_ADDR, getHost());
    }

    public int getFrpServerPort() {
        return sharedPreferences.getInt(KEY_FRP_SERVER_PORT, 7000); // Default frps port.
    }

    public String getFrpToken() {
        return sharedPreferences.getString(KEY_FRP_TOKEN, ""); // Default empty token.
    }
}